...
const char* ssid = "myESP32";       // Enter SSID here
...
WebServer server(80);  // port at which the server communicates with the clients
...
void setup() {
  ...
  WiFi.softAP(ssid, password);                   // Setup the SSID and Password, for other devices to connect to the ESP32
  WiFi.softAPConfig(local_ip, gateway, subnet);  // We ned to give an IP address to the Web Server, so that the connected devices can use it
